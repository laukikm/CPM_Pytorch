...
class CarDataset(Dataset):
    ...
    def __init__(self, data_dir, n_keypoints=18, transform=None, sigma=1):
        ...
        self.images_dir=sorted(glob.glob(data_dir+"Images/*.PNG"),key=HumanSortKey) #It's okay if they aren't in the human order
        self.label_dir = sorted(glob.glob(data_dir+"Labels/*.txt"),key=HumanSortKey)# But they need to be in the same order
